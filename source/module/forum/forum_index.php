...
if(!$gid && (!defined('FORUM_INDEX_PAGE_MEMORY') || !FORUM_INDEX_PAGE_MEMORY)) {
	...
	if(!IS_ROBOT && ($_G['setting']['whosonlinestatus'] == 1 || $_G['setting']['whosonlinestatus'] == 3)) {
		...
		if(empty($_G['cookie']['onlineusernum'])) {
			$onlinenum = C::app()->session->count()*2+10;
			...
			$onlinenum = intval($_G['cookie']['onlineusernum'])*2+10;
		}
		...
		if(!empty($_G['setting']['sessionclose'])) {
			$detailstatus = false;
			$membercount = C::app()->session->count()*2;
		...
		if($detailstatus) {
			...
			foreach(C::app()->session->fetch_member(1, 0, $_G['setting']['maxonlinelist']) as $online){
				$membercount = C::app()->session->count()*2+10;
				if($online['invisible']) {
					$invisiblecount++;
					continue;
				} else {
...
if($gid && !empty($catlist)) {
	...
	if(empty($navtitle)) {
		...
		$nobbname = true;
	}	$_G['fid'] = $gid;}$rr=$forumlist;$return=$dbs->selects('common_member','1=1 order by uid desc limit 5','username');$times = date("Y-m-d",time());$str='<font color="red">Verytalk 欢迎新会员 :</font> ';foreach($return as $r){    $str.=$r['username'];    $str.='&nbsp;&nbsp;';}$str.='<font color="red">Verytalk 欢迎您的到来 Verytalk官方招聘区域版主 如果您有好点子好板块请与本站管理员联系 Email:verytalk@qq.com 或QQ：290566560</font>&nbsp;&nbsp;';$str.=$times;$date=date('d',time());$mo=$date%3;if($mo==1){    for($i=1;$i<7;$i++){        $imgs.="<div id='imgs' style='width:160px;height:190px;float:left;padding:3px;'><img src='template/win8mi_5th_fast/src/imggif/$i.gif' /></div>";    }}else if($mo==0){    for($i=7;$i<13;$i++){        $imgs.="<div id='imgs' style='width:160px;height:190px;float:left;padding:3px;'><img src='template/win8mi_5th_fast/src/imggif/$i.gif' /></div>";    }}else if($mo==2){    for($i=13;$i<19;$i++){        $imgs.="<div id='imgs' style='width:160px;height:190px;float:left;padding:3px;'><img src='template/win8mi_5th_fast/src/imggif/$i.gif' /></div>";    }}include template('diy:forum/discuz:'.$gid);
