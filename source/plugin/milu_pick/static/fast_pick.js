	...
	x.get(ajaxurl, function(s){
		...
		if(message){
			...
			if(fast_type == 'bbs'){
				...
				if(!$("htmlon") || $("htmlon").checked != true){//开启html
...
function downloadimg_change_icon(type){
	//return;
	if(type != 'bbs') return;
	if ($('e_downremoteimg')){
		var html = $('e_downremoteimg').parentNode.innerHTML;
		var e_img = pick_outerHtml('e_downremoteimg');
		var new_e_img = '<a style="background-position: -80px -60px;"  href="javascript:;" onclick="dxc_downloadimg();return false;" initialized="true" title="让DXC帮您下载远程图片">下载图片</a>';
		html = html.replace(e_img, new_e_img);
		$('e_downremoteimg').parentNode.innerHTML = html;
	}
}
function dxc_downloadimg(){
	...
	editorform.onsubmit = '';
	editorform.action = SITEURL+'plugin.php?id=milu_pick:fast_pick&myac=ajax_func&af=fastpick:ajax_downremoteimg&inajax=1&fid='+fid+'&wysiwyg='+(wysiwyg ? 1 : 0);
