...
$j(document).ready(function(){
	$j("#J_time_pick_select").change(function(){
		$j('#J_time_pick_'+ $j(this).val()).show().siblings('.J_time_pick_item').hide();
	});
	
	$j("#J_time_timing_select").change(function(){
		$j('#J_time_timing_'+ $j(this).val()).show().siblings('.J_time_timing_item').hide();
	});
	
	$j("#J_time_auto_update_select").change(function(){
		$j('#J_time_auto_update_'+ $j(this).val()).show().siblings('.J_time_auto_update_item').hide();
	});
...
var RPC_FUNCTIONS = ['test_window', 'load_keyword', 'show_rules_set', 'login_test', 'weibo_add','load_temp', 'tran_api_test'];
...
function save_to_pick222(){
	$j('#save_to_pick').val(1);
}
...
function show_page_setting(){
	_fiter_show('is_fiter_page_link', 'page_link_fiter_replace');
	_fiter_show('is_pick_cover_from_listpage', 'pick_cover_rules_show');
}
...
function format_url(url){
	...
	url = url.replace(reg1,'JK123LL');
	url = url.replace(reg2,'JK123GG');
	url = url.replace(reg3,'CYH123');
...
function check_web_type(id,test){
	...
	if(!list_ID || list_ID.length < 20) {
		$j("#"+test+"_show").html('<font style="color:red;">×特征字符串长度不够</font>', 'notice');
		return;
	}
	$j("#"+test+"_show").html(loading_html('正在检测'));
	$j.post(PICK_URL+'system_rules&inajax=1&myac=check_web_type&tpl=no&url='+url+'&list_ID='+list_ID+'&charset_type='+pcharset_type(),null,function (msg) {
		if(msg == 1){
...
function cat_select_picker(cid){
	$j.post(PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=picker_list_select&cid='+cid,null,function (msg) {
		//if(!msg) msg = '';
		$j("#picker_select_show").show();
		$j("#picker_select_show").html(msg);
	});
}


function skydrive_select(value){
	$j.post(PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=skydrive_output&type='+value,null,function (msg) {
		$j("#skydrive_show").show();
		$j("#skydrive_show").html(msg);
	});
}

function skydrive_test(type){
	var a = b = c = d = '';
	showDialog('', 'info', loading_html('请稍候'));
	if(type == 1){//百度
		a=format_url($j('#baidu_bucket').val());
		b=format_url($j('#baidu_ak').val());
		c=format_url($j('#baidu_sk').val());
	}else if(type == 2){//七牛云
		a=format_url($j('#qiniu_bucket').val());
		b=format_url($j('#qiniu_ak').val());
		c=format_url($j('#qiniu_sk').val());
		d=format_url($j('#qiniu_domain').val());
	}
	$j.post(PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=skydrive_test&type='+type+'&a='+a+'&b='+b+'&c='+c+'&d='+d,null,function (msg) {
		if(msg == 1){
			showDialog("一切正常", 'right');
		}else{
			showDialog("无法通过测试，请检查配置信息", 'notice');
		}
	});
}
...
function rules_get_test(v){
	var get_type = $j(".set_"+v+"_get_type_:radio:checked").val();
	var get_rules = format_url($j("#set_"+v+"_get_rules_").val());
	var url = (format_url($j("#page_url_test").val()));
	var login_cookie = get_login_cookie();
	showWindow('article_detail', PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=rules_get_test&tpl=no&get_type='+get_type+'&get_rules='+get_rules+'&func='+v+'&login_cookie='+login_cookie+'&url='+url+'&charset_type='+pcharset_type());
}

function pcharset_type(){
	return $j(".charset_type:radio:checked").val();
}

function get_page_link(){
	is_page_fiter = $j(".page_fiter:radio:checked").val();
	is_filter = $j(".is_fiter_page_link:radio:checked").val();
	get_type = $j(".page_get_type_dom:radio:checked").val();
	var page_url_no_other = '';
	if(is_page_fiter == 1) {
		page_url_no_other = format_url($j('#page_url_no_other').val());
	}
	page_url_replace = format_url($j('#page_link_replace_rules').val());
	...
	showWindow('article_detail', PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=system_get_link_test&tpl=no&is_filter='+is_filter+'&&is_page_fiter='+is_page_fiter+'&type='+get_type+'&page_url_no_other='+page_url_no_other+'&page_url_contain='+page_url_contain+'&page_url_no_contain='+page_url_no_contain+'&b='+page_link_rules+'&c='+page_url_test+'&login_cookie='+login_cookie+'&page_url_replace='+page_url_replace+'&charset_type='+pcharset_type());
}
...
function test_window(get_type, is_fiter, url_test, rule, replace_rule, id_name, show_type){
	...
	var login_cookie = format_url($j("#login_cookie").val());
	var best_answer_get_type = ask_reward_price_get_type = 0 ;
	var best_answer_get_rules = best_answer_flag = ask_reward_price_get_rules = '';
	...
	if(show_type  == 'reply'){
		...
		if($j(".is_setting_best_answer:radio:checked").val() == 1){
			best_answer_get_type = $j(".set_best_answer_get_type_:radio:checked").val();
			best_answer_get_rules = $j('#set_best_answer_get_rules_').val();
			best_answer_flag = $j('#best_answer_flag').val();
			ask_reward_price_get_type = $j(".set_ask_reward_price_get_type_:radio:checked").val();
			ask_reward_price_get_rules = $j("#set_ask_reward_price_get_rules_").val();
		}
	...
	rpcClient.test_window(get_type, url_test, is_fiter, rule, replace_rule, filter_data, show_type,login_cookie, filter_html_arr, reply_is_extend, best_answer_get_type, best_answer_get_rules, best_answer_flag, ask_reward_price_get_type, ask_reward_price_get_rules, pcharset_type(), rpcCallback);
}
...
function show_user_setting(a, b){
	check_flag = $j("."+a+":checkbox:checked").val();
	if(check_flag == 1){
		show_hide(b,'',2);
	}else{
		show_hide(b,'',1);
...
function show_html_window(k, title, width, height, html, c){
	...
	show_html = '<h3 class="flb"><div style="float:left">'+title+'</div><span style="float:right"><a href="javascript:;" class="flbc" onclick="hideWindow(\''+k+'\');" title="关闭">点击就关闭窗口</a></span></h3><div class="article_detail c" style="width:'+width+'px;height:'+height+'px;overflow-y:scroll; '+c_html+'; line-height:23px;">'+ html+'</div>';
	showWindow(k, show_html, 'html', '',{'maxh':'800'});
}
			...
			if (result == false) {
				...
				if(output){
					...
					if(obj){
						...
						var charset_type = strdecode(obj.charset_type);
						if(charset_type == 0) charset_type = 1;
						$j(".charset_type:radio:checked").parent().parent().removeClass('checked');
						$j(".charset_type:radio:checked").attr("checked", false);
						$j("#charset_type_"+charset_type).attr("checked",true);
						$j("#charset_type_"+charset_type).parent().parent().addClass("checked");
...
function rules_show_page_set(type){
	...
	if(type == 1){//规则模板
		...
		_fiter_show('is_get_threadtypes', 'show_get_threadtypes');
		_fiter_show('is_get_threadtypes', 'show_threadtypes_html');
		_fiter_show('is_fiter_content_page_link', 'content_page_link_fiter_replace');
		_fiter_show('is_attach_setting', 'attach_setting_show');
		
		
		_fiter_show('is_get_thread_user', 'show_get_thread_user');
		$j("#get_page_link_test").show();
		show_reply_setting();//回复
		show_aritcle_page_setting();//分页
		
		show_get_best_answer();//问答
		show_article_cover();//封面
		...
		if(url_range_type > 1) $j("#get_page_link_test").hide();
		$j("#show_common_get_content tbody").show();
		$j("#show_get_other").hide();
		$j("#show_get_threadtypes").hide();
		_fiter_show('is_fiter_title', 'title_fiter_replace');
		_fiter_show('is_fiter_content', 'content_fiter_replace');
		_fiter_show('is_fiter_reply', 'reply_fiter_replace');
		_fiter_show('keyword_flag', 'show_keyword_filter');
...
function show_reply_setting(){
	var id_str = 'get_reply_show+reply_show+is_ask_show+is_format_reply_show+max_replies_show+is_get_post_show+show_get_post_use+show_get_post_user_rules_test+show_get_post_user_rules+reply_test_show+show_get_post_user+reply_fiter_replace+show_get_best_answer';
	if($j(".is_get_reply:radio:checked").val() == 2){//否
		show_hide(id_str, '',2);
	}else{
		show_hide(id_str, '',1);
		show_post_info_setting();
		show_get_best_answer();//问答
		_fiter_show('is_fiter_reply', 'reply_fiter_replace');//进一步处理回复
		_fiter_show('is_setting_best_answer', 'best_answer_get_rules');//是否设置问答
		if($j(".reply_is_extend:checkbox:checked").val() == 1){//继承内容规则
...
//文章封面
function show_article_cover(){
	if($j(".url_range_type:radio:checked").val() == 1 && $j(".rules_type:radio:checked").val() != 3){//显示
		show_hide('show_get_page2', '',1);
	}else{
		show_hide('show_get_page2', '',2);
	}
...
//文章分页是否显示
function show_aritcle_page_setting(){
	var id_str = 'article_page_show1+article_page_show_test+content_page_link_fiter_replace';
	if($j(".is_setting_article_page:radio:checked").val() == 2){//否
		show_hide(id_str, '',2);
	}else{
		show_hide(id_str, '',1);
		_fiter_show('is_fiter_content_page_link', 'content_page_link_fiter_replace');//进一步处理网址
	}
}

function show_post_info_setting(){
	if($j(".is_get_reply:radio:checked").val() == 2){//否
		return;
	}
	_fiter_show('is_get_post_user', 'show_get_post_user');
	_fiter_show('is_get_post_user', 'show_get_post_user_rules');
	check_flag = $j(".is_use_thread_setting:checkbox:checked").val();
	if(check_flag == 1 || $j(".is_get_post_user:radio:checked").val() == 2){
		show_hide('show_get_post_user_rules','',2);
	}else{
		show_hide('show_get_post_user_rules','',1);
	}
}

//最佳答案
function show_get_best_answer(){
	if($j(".is_get_reply:radio:checked").val() == 2){//否
		return;
	}
	if($j(".is_setting_best_answer:radio:checked").val() == 2){//否
		show_hide('show_get_best_answer', '', 2);
		return;
	}
	
	
	var check = $j(".set_best_answer_get_type_:radio:checked").val();
	var id_str = 'best_answer_desc3+best_answer_flag_rules_name+best_answer_flag_rules';
	if(check == 3){//从回复里面取
		show_hide(id_str,'',1);
	}else{
		show_hide(id_str, 'show_get_best_answer',2);
	}
	
	var check = $j(".set_ask_reward_price_get_type_:radio:checked").val();

	var id_str = 'ask_reward_price_desc3';
	if(check == 3){//自定义
		
		show_hide(id_str, 'ask_reward_price_desc1',1);
	}else{
		show_hide(id_str, 'show_get_best_answer+ask_reward_price_desc1',2);
	}
	
}

//是否采集回帖用户信息
function is_get_post_user_show(){
	var id_str = 'show_get_post_user+show_get_post_user_rules';
	if($j(".is_get_post_user:radio:checked").val() == 2){//否
		show_hide(id_str,'',2);
	}else{
		if($j(".is_use_thread_setting:checkbox:checked").val() == 1){//继承
			show_hide('show_get_post_user_rules','show_get_post_user',2);
			return;
		}
		show_hide(id_str,'',1)
	}
...
function forum_threadtype_select(typeid, showid, pre, rule_type, rule_id){
	$j.post(PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=forum_threadtype_html&inajax=1&inajax=1&typeid='+typeid+'&pre='+pre+'&charset_type='+pcharset_type()+'&rule_type='+rule_type+'&rule_id='+rule_id,null,function (msg) {
		if(!msg) msg = '<tr><td>此分类尚未设置字段</td></tr>';
		$j("#"+showid).show();
		$j("#"+showid).html(msg);
	});
}

function get_user_info_test(v){
	var login_cookie = get_login_cookie();
	var theme_url_test = format_url($j("#theme_url_test").val());
	var is_use_thread_setting = 0;
	is_use_thread_setting = $j(".is_use_thread_setting:checkbox:checked").val();
	//
	
	var user_get_type = $j(".set_thread_user_get_type_:radio:checked").val();
	var user_get_rules = format_url($j("#set_thread_user_get_rules_").val());
	var dateline_get_type = $j(".set_thread_dateline_get_type_:radio:checked").val();
	var dateline_get_rules = format_url($j("#set_thread_dateline_get_rules_").val());
	
	var is_get_user_other = $j(".is_get_user_other:radio:checked").val();
	var user_other_rules = format_url($j("#user_other_rules").val());
	
	//
	var type = 'thread';
	if(v != 'thread_dateline'){//回复
		type = 'reply';
		if(is_use_thread_setting != 1){//单独设置
			user_get_type = $j(".set_post_user_get_type_:radio:checked").val();
			user_get_rules = format_url($j("#set_post_user_get_rules_").val());
			dateline_get_type = $j(".set_post_dateline_get_type_:radio:checked").val();
			dateline_get_rules = format_url($j("#set_post_dateline_get_rules_").val());
		}	
			
	}
	if(is_get_user_other == 2) user_other_rules = '';
	showWindow('kk', PICK_URL+'picker_manage&myfunc=ajax_func&inajax=1&af=get_user_info_test&tpl=no&xml=1&user_get_type='+user_get_type+'&user_get_rules='+user_get_rules+'&dateline_get_type='+dateline_get_type+'&dateline_get_rules='+dateline_get_rules+'&login_cookie='+login_cookie+'&theme_url_test='+theme_url_test+'&type='+type+'&is_use_thread_setting='+is_use_thread_setting+'&is_get_user_other='+is_get_user_other+'&user_other_rules='+user_other_rules+'&charset_type='+pcharset_type());
}


function get_url_by_listpage_rand(v){
	var error = '<font class="red">获取不到网址,请手动输入</font>';
	var success = '<font style="color:Green;">√</font>';
	$j("#show_status_"+v).html(loading_html('请稍候'));
	...
	$j("#theme_url_test_notice").hide();
	if(v == 'page_url_test'){
				
		if(auto == 'undefined') auto = 0;
		if(!url){
			$j("#show_status_"+v).html(error);
			return;
		}
		$j.post(PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=url_page_range_test&inajax=1&tpl=no&url='+url+'&auto='+auto+'&start='+start+'&end='+end+'&step='+step+'&rand=1'+'&charset_type='+pcharset_type(),null,function (msg) {
			if(!msg || typeof(msg) == "object"){
				$j("#show_status_"+v).html(error);
				return;
			}
			$j("#page_url_test_visit").show();
			$j("#page_url_test_visit").attr('href', msg);
			$j("#show_status_"+v).html(success);
			$j("#page_url_test").val(msg);
		});
	}else if(v == 'theme_url_test'){
		url_range_type = $j(".url_range_type:radio:checked").val();
		if(url_range_type == 1){//从列表
			...
			page_url_test = format_url($j('#page_url_test').val());
			var login_cookie = format_url($j("#login_cookie").val());
			$j.post(PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=system_get_link_test&tpl=no&is_filter='+is_filter+'&type='+get_type+'&page_url_no_other='+page_url_no_other+'&page_url_contain='+page_url_contain+'&page_url_no_contain='+page_url_no_contain+'&b='+page_link_rules+'&c='+page_url_test+'&login_cookie='+login_cookie+'&rand=1'+'&charset_type='+pcharset_type(),null,function (msg) {
				get_rand_url_show(msg, error, v, success);
			});
		}else if(url_range_type == 2){//从详细页
			if(!url){
				$j("#show_status_"+v).html(error);
				return;
			}
		...
		}else if(url_range_type == 4){//从rss
			var rss_url = encodeURIComponent(format_url($j('#rss_url').val()));
			$j.post(PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=get_rss_url&rss_url='+rss_url+'&rand=1'+'&charset_type='+pcharset_type(),null,function (msg) {
				get_rand_url_show(msg, error, v, success);
			});
...
function get_rand_url_show(msg, error, v, success){
  	if(!msg || typeof(msg) == "object"){
		$j("#show_status_"+v).html(error);
		return;
	}
	$j("#theme_url_test_visit").show();
	$j("#theme_url_test_visit").attr('href', msg);
	$j("#show_status_"+v).html(success);
	$j("#theme_url_test").val(msg);
}
...
function url_page_range_test(){
	...
	step = $j("#page_url_auto_step").val();
	showWindow('kk', PICK_URL+'system_rules&myac=url_page_range_test&inajax=1&tpl=no&url='+url+'&auto='+auto+'&start='+start+'&end='+end+'&step='+step+get_filter_url_str());
}
...
function many_list_test(id){
	...
	//过滤
	is_page_fiter = $j(".page_fiter:radio:checked").val();
	var is_filter = $j(".is_fiter_page_link:radio:checked").val();
	var page_url_no_other = '';
	if(is_page_fiter == 1) {
		page_url_no_other = format_url($j('#page_url_no_other').val());
	}
	var page_url_replace = format_url($j("#page_link_replace_rules").val());
	var page_url_contain = format_url($j('#page_url_contain').val());
	var page_url_no_contain = format_url($j('#page_url_no_contain').val());
	
	showWindow('kk', PICK_URL+'system_rules&tpl=no&myac=many_list_test&type='+type+'&rules='+rules+'&test='+test+'&login_cookie='+login_cookie+'&charset_type='+pcharset_type()+get_filter_url_str());
}

function get_filter_url_str(){
	//过滤
	is_page_fiter = $j(".page_fiter:radio:checked").val();
	var is_filter = $j(".is_fiter_page_link:radio:checked").val();
	var page_url_no_other = '';
	...
	var page_url_contain = format_url($j('#page_url_contain').val());
	var page_url_no_contain = format_url($j('#page_url_no_contain').val());
	return '&is_filter='+is_filter+'&is_page_fiter='+is_page_fiter+'&page_url_contain='+page_url_contain+'&page_url_no_contain='+page_url_no_contain+'&page_url_replace='+page_url_replace+'&page_url_no_other='+page_url_no_other;
}


function get_rss_link(){
	var rss_url = encodeURIComponent(format_url($j('#rss_url').val()));
	showWindow('kk', PICK_URL+'system_rules&tpl=no&inajax=1&myac=get_rss_url&rss_url='+rss_url+get_filter_url_str());
}
function cnCode(str) {
	str = str.replace(/<\/?[^>]+>|\[\/?.+?\]|"/ig, "");
...
function check_uid(uid){
	if(!uid) return false;
	$j.post(PICK_URL+'picker_manage&inajax=1&myfunc=ajax_func&af=pick_check_uid_exists&tpl=no&xml=0&uid='+uid,null,function (msg) {
		if(msg != 'no'){
...
function attach_download_url_test(v){
	var login_cookie = get_login_cookie();
	var attach_redirect_url_get_type = $j(".set_attach_redirect_url_get_type_:radio:checked").val();
	var attach_redirect_url_get_rules = format_url($j("#set_attach_redirect_url_get_rules_").val());
	
	var attach_download_url_get_type = $j(".set_attach_download_url_get_type_:radio:checked").val();
	var attach_download_url_get_rules = format_url($j("#set_attach_download_url_get_rules_").val());
	
	var url = format_url($j("#theme_url_test").val());

	showWindow('kk', PICK_URL+'picker_manage&myfunc=ajax_func&inajax=1&af=attach_download_url_test&tpl=no&xml=1&attach_redirect_url_get_type='+attach_redirect_url_get_type+'&attach_redirect_url_get_rules='+attach_redirect_url_get_rules+'&attach_download_url_get_type='+attach_download_url_get_type+'&attach_download_url_get_rules='+attach_download_url_get_rules+'&login_cookie='+login_cookie+'&url='+url+'&charset_type='+pcharset_type());
}
...
function get_content_page(){
	...
	var login_cookie = get_login_cookie();
	var is_fiter = $j(".is_fiter_content_page_link:radio:checked").val();
	var content_page_link_replace_rules = format_url($j("#content_page_link_replace_rules").val());
	var content_page_url_contain = format_url($j("#content_page_url_contain").val());
	var content_page_url_no_contain = format_url($j("#content_page_url_no_contain").val());
	showWindow('kk', PICK_URL+'picker_manage&myfunc=ajax_func&inajax=1&af=system_get_link_test&tpl=no&xml=1&type='+get_type+'&b='+rules+'&page_type='+page_type+'&c='+url+'&is_filter='+is_fiter+'&dom_type=content_page&login_cookie='+login_cookie+'&is_fiter='+is_fiter+'&page_url_contain='+content_page_url_contain+'&page_url_no_contain='+content_page_url_no_contain+'&page_url_replace='+content_page_link_replace_rules+'&charset_type='+pcharset_type());
}
...
function login_test(){
	...
	var login_cookie = format_url($j("#login_cookie").val());
	var must_have = format_url($j("#cookie_test_hava").val());
	var no_hava = format_url($j("#cookie_test_no_hava").val());
	...
	$j.post(PICK_URL+'picker_manage&myfunc=ajax_func&inajax=1&af=login_test&tpl=no&xml=0&is_login='+is_login+'&login_test_url='+login_test_url+'&must_have='+must_have+'&no_hava='+no_hava+'&charset_type='+pcharset_type(), { login_cookie: login_cookie},function (msg) {
		hideMenu('fwin_dialog', 'dialog');
		show_html_window('aa', 'cookie登录测试', 450, 320, msg);
	})
...
function get_user_info(){
	...
	showWindow('kk', PICK_URL+'member&myac=ajax_func&inajax=1&af=get_member_info&tpl=no&login_cookie='+login_cookie+'&url='+url+'&uid_range='+uid_range+'&test_uid='+test_uid+'&charset_type='+pcharset_type());
}
...
function p_finsh(){
	$j(".show_loading").remove();
	var clear_log = $j('#pid').length ? '<input name="no_check_url" style="margin:10px 10px 0 10px ; " type="checkbox" value="1" />重复采集' : '';
...
function get_other_test(){
	...
	showWindow('kk', PICK_URL+'picker_manage&myac=ajax_func&inajax=1&af=get_other_test&tpl=no&login_cookie='+login_cookie+'&url='+url+'&from_get_type='+from_get_type+'&author_get_type='+author_get_type+'&dateline_get_type='+dateline_get_type+'&from_get_rules='+from_get_rules+'&author_get_rules='+author_get_rules+'&dateline_get_rules='+dateline_get_rules+'&charset_type='+pcharset_type());
}

function rules_get_threadtypes(name){
	var url = format_url($j("#theme_url_test").val());
	var login_cookie = get_login_cookie();
	var get_type = $j(".forum_threadtypes_get_type__"+name+"_:radio:checked").val();
	
	var get_rules = format_url($j("#forum_threadtypes_get_rules__"+name+"_").val());
	showWindow('kk', PICK_URL+'picker_manage&myac=ajax_func&inajax=1&af=rules_get_threadtypes&tpl=no&login_cookie='+login_cookie+'&url='+url+'&get_type='+get_type+'&get_rules='+get_rules+'&charset_type='+pcharset_type());
}
...
function rules_trun(id, type){
	...
	select_arr[1] = '<option value="system">规则模板</option>';
...
function import_threadtype_data(obj, type, data_id){
	showWindow(obj.id, PICK_URL+'picker_manage&myac=ajax_func&inajax=1&af=import_threadtype_data&tpl=no&type='+type+'&data_id='+data_id, 'get', 0);
}
function import_threadtype_data_ajax(type, data_id){
	hideWindow('import_threadtype_data_show');
	$j.post(PICK_URL+'picker_manage&myac=ajax_func&inajax=1&af=import_threadtype_data&tpl=no&submit=1&type='+type+'&data_id='+data_id, null,function (msg) {
		if(msg > 0){
			var select_html = $j('#forum_threadtype').html()+'<option selected="selected" value="'+msg+'">'+$j('#sort_name_show').html()+'</option>';
			$j('#import_threadtype_data_show_span').html('');
			$j('#forum_threadtype').html(select_html);
			forum_threadtype_select(msg, 'show_threadtypes_html', 'forum', type, data_id);
			showDialog("导入成功", 'right');
		}else{
			showDialog("信息分类导入失败，分类选项变量名重复", 'notice');
			return;
		}
	})
}