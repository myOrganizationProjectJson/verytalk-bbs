...
function member_set(){
	...
	if(!submitcheck('addsubmit')) {
		...
		$set = $_POST['set'];
		$set['num'] = intval($set['num']); 
		$set['jump_num'] = intval($set['jump_num']); 
		$set['member_field'] = pserialize($_POST['member_field']);
		pick_common_set($set);
...
function member_get($run = 0){
	...
	if(!$_GET['submit'] || $run == 0) {
		num_limit('strayer_member', '1500', 'member_num_limit');
		$info['header'] = pick_header_output($header_config, $head_url);
		...
		if($success_count < $get_count && $info['reg_num']) {
			if($info['jump_num'] > $get_count && $now_get != $get_count){
				cache_data('member_get', $temp_cache_data, 3600);
				data_go('member&myac=member_get&submit=1');
				exit();
			}
...
function member_edit(){
	...
	if(!submitcheck('editsubmit')) {
		...
		$info['bloodtype_select'] = pickOutput::select_output(array('A', 'B', 'AB', milu_lang('other')), milu_lang('other'), 'set[bloodtype]', $info['bloodtype']);
		$info['education_select'] = pickOutput::select_output(array(milu_lang('boshi'), milu_lang('shuoshi'), milu_lang('benke'), milu_lang('zuanke'), milu_lang('zhongxue'), milu_lang('xiaoxue'), milu_lang('other')), milu_lang('other'), 'set[education]', $info['education']);
...
function member_public_set(){
	...
	if(!$_GET['submit']) {
		$info = pick_common_get();
		$info = dhtmlspecialchars($info);
		$info['reg_pwd'] = $info['reg_pwd'] == 'dfG56#$fg' ? '' : $info['reg_pwd'];
		$info['header'] = pick_header_output($header_config, $head_url);
		$info['credits_list'] = credits_list();
		...
		$info['pickOutput::user_group_select'] = pickOutput::user_group_select('set[public_groupid]', unserialize($info['public_groupid']));
		return $info;
	}else{
		$set = $_POST['set'];
		$set['reg_num'] = intval($set['reg_num']); 
		$set['reg_jump_num'] = intval($set['reg_jump_num']); 
		
		if(!$set['reg_pwd']) cpmsg_error(milu_lang('no_pwd'));
		if(strlen($set['reg_pwd']) < 6) cpmsg_error(milu_lang('pwd_too_short'));
...
function member_public_info(){
	...
	if(!$_GET['confirmed']){
		...
		echo pickOutput::pick_loading($ajax_url, $finsh_url, $total, array('bat_num' => 500));
	}else{
		ob_end_clean();
...
<tr class="hover"><td><div class="tipsblock"><ul id="tipslis"><li>'.milu_lang('user_data_download_notice').'</li ><li>'.milu_lang('user_data_size').':'.$msg_arr->size.'</li ><li>'.milu_lang('update_dateline').': '.dgmdate($msg_arr->modify_dateline).' </li ><li>'.milu_lang('member_count').':  '.$msg_arr->count.milu_lang('tiao').'</li ></ul></div></td><td><input type="button" class="btn" onclick="window.location.href=\'?'.$go_url.'&step=2&tpl=no&c='.$msg_arr->count.'\'" value="'.milu_lang('confirm_download').'"></td></tr></tbody></table>';
		...
		$img_arr = get_img_content($url.'&get_type=2&v='.urlencode(PICK_VERSION), $snoopy_obj, array('no_only_image' => 1));
		$data_text = $img_arr['content'];
		if(!$data_text) cpmsg_error(milu_lang('download_member_fail'));
		...
		$per_num = 250;
		pickOutput::pcpmsg_loading(milu_lang('importing'), PICK_GO."member&myac=member_import_online&tpl=no&step=3&finished=1", 'loadingform', '', '<div id="percent">0%</div>', FALSE);
		...
		echo pickOutput::pick_loading($ajax_url, $finsh_url, $_GET['c'], array('bat_num' => $per_num));
	}else if($step == 4){
...
function member_public($run = 0){
	...
	if(!$_GET['submit'] || $run == 0) {
		...
		$info['reg_pwd'] = $info['reg_pwd'] == 'dfG56#$fg' ? '' : $info['reg_pwd'];
		if(!$info['reg_pwd']) {
			show_pick_info(milu_lang('no_pwd'), 'exit');
			return FALSE;
		}
		if(strlen($info['reg_pwd']) < 6) {
			show_pick_info(milu_lang('pwd_too_short'), 'exit');
			return FALSE;
		}
...
function avatar_set(){
	...
	if(!$_GET['submit']) {
		...
		$set = $_POST['set'];
		$set['avata_jump_num'] = intval($set['avata_jump_num']);
		$set['avata_from_uid'] = intval($set['avata_from_uid']);
		pick_common_set($set);
...
function avatar_get($run = 0){
	...
	if(!$_GET['submit'] || $run == 0) {
		...
		$info['avata_from_uid'] = $info['avatar_from_type'] == 1 ? 1 : $info['avata_from_uid'];
...
function get_web_avatar($args, $i = 0){
	...
	$server_icon_root_url = GET_URL.'plugin.php?id=pick_user:api&tpl=no&myac=api_download_avatar&inajax=1&get_uid='.$avatar_get_uid.'&size=';
	$icon_url = $info['avatar_from_type'] == 1 ? $server_icon_root_url.'middle' : $icon_url;
	...
	$img_re = (array)get_img_content($icon_url, $snoopy_obj);//得到的是middle的头像
	$img_content = $img_re['content'];
	...
	if(!$img_re || $img_re['file_size'] == 3972 || $img_re['file_size'] == 2421 || $img_re['file_size'] < 1000){//3972是discuz默认头像的大小
		...
		foreach($size_arr as $size){
			if($size != 'middle') {
				$icon_url = $info['avatar_from_type'] == 1 ? $server_icon_root_url.$size : $avatar_web_url.get_avatar($avatar_get_uid, $size);
				$img_re = (array)get_img_content($icon_url, $snoopy_obj);
				$img_content = $img_re['content'];
					
			}
			$avatar_dir_save = $uc_server_dir.'/'.get_avatar($uid, $size);
			...
			$put_re = $img_content ? file_put_contents($avatar_dir_save, $img_content) : FALSE;//写入头像
			if(!$put_re || !$img_re || $img_re['file_size'] == 3972 || $img_re['file_size'] == 2421 || $img_re['file_size'] < 1000) {
				show_pick_info($img_content ? milu_lang('avatar_dir_no_wirte2') : milu_lang('avatar_dir_no_wirte2'), 'err', $show_arr);
				$info['now_get']++;
				$info['avatar_get_uid']++;
				avatar_page_jump($now_get, $avata_jump_num, $get_count);
				return get_web_avatar($info, $i + 1); 
...
function pick_get_user_info($args){
	...
	$user_info = dom_get_str($html, 'div.hm h2.xs2 a', array('is_get_all' => 1, 'text_type' => 'innertext', 'is_return_array' => 1));
	if($user_info){
		$username = _striptext($user_info[0]);
		$username = str_iconv(trim($username));
	}
	if(!$info_arr || empty($username)) {
	...
	if($info_field_arr && (count($must_arr) != count($info_field_arr) || !$username )) return -3;//取不到所有必须资料
...
function format_member_data($data_arr){
	foreach($data_arr as $k2 => $value){
		...
		if($k2 == 'qq'){
			if(strexists($value, 'wpa.qq.com')){
				preg_match_all('/Uin=(.+\d)&/i',$value, $matchs);
				$re_arr[$k2] = $matchs[1][0];
			}
			$re_arr[$k2] = intval($re_arr[$k2]);
		}
		//个人主页
...
function get_member_info(){
	...
	$re_arr[milu_lang('user_name')] =  $username;
	if(!$info_arr){
...
function no_member_tips(){
	...
	if($check) return;
	return pickOutput::show_tips('<div class="tipsblock"><li>'.milu_lang('no_member_tips', array('url' => '?'.PICK_GO.'member&myac=member_import_online&tpl=no')).'</li></ul></div>', array('key' => 'no_user', 'w' => 380, 'h' => 250));
