...
function get_client_info(){
	...
	$re['httpservice'] = function_exists('_create_service') ? _create_service(1) : '';
	return $re;
}
...
//取得网站的密钥
function pick_get_site_key(){
	global $_G;
	$client_info = get_client_info();
	if(!$client_info || !$client_info['siteurl']) cpmsg_error(milu_lang('server_no_use'));
	$cache_dir = DISCUZ_VERSION != 'X2' ? 'sysdata' : 'cache';
	$cachefile = DISCUZ_ROOT.'./data/'.$cache_dir.'/milu_pick_key.php';
	if(file_exists($cachefile)){
		@include $cachefile;
		$data = unserialize($c_data);
		$data['version'] = PICK_VERSION;
		if($data['domain'] == $client_info['domain'] && $data['key_code']) return $data;
	}
	$siteurl = $client_info['siteurl'];
	$key_file = PICK_PATH.'/data/milu_pick_key.php';
	$client_key = md5($siteurl);
	file_put_contents($key_file, $client_key);
	if(!file_exists($key_file)) cpmsg_error(milu_lang('dir_no_write', array('dir' => 'source\plugin\milu_pick\data')));
	$msg = dfsockopen(GET_URL.'plugin.php?id=pick_user:api&tpl=no&myac=get_key&inajax=1&client_key='.$client_key.'&siteurl='.urlencode($siteurl));
	$msg = pick_ajax_decode($msg);
	if($msg->status == -4) cpmsg_error(milu_lang('key_code_err4', array('file' => 'source\plugin\milu_pick\data\milu_pick_key.php'))); 
	if($msg->status < 1) cpmsg_error(milu_lang('key_code_err', array('c' => $msg->status)));  
	@unlink($key_file);
	$key_code = $msg->key_code;
	$c_data = array('domain' => $client_info['domain'], 'key_code' => $key_code);
	writetocache('milu_pick_key', getcachevars(array('c_data' => serialize($c_data))),'');
	$c_data['version'] = PICK_VERSION;
	return $c_data;
}
...
function version_check(){
	...
	if($result == 'ok'){
		return pickOutput::show_tips('<div class="tipsblock"><li>'.milu_lang('have_new_version', array('url' => '?'.PICK_GO.'pick_info&ac=pick_check&checking=1')).'</li></ul></div>', array('key' => 'check_version', 'w' => 380, 'h' => 250));
	}
}
...
function pick_check(){
	...
	$zend_check = is_zend();
	if($zend_check == -1){
		cpmsg_error(lang('plugin/milu_pick','http_visit', array('file' => 'api/addons/zendcheck.php')) );
	...
	if(!is_array($msg_arr) && (empty($msg_arr) || intval($msg_arr) < 0)) cpmsg_error(milu_lang('no_conn_up'));
	...
	if($status == -1) {
		...
		cpmsg_error(milu_lang('no_normal_up').$status);
	}else if($status == -5){
		cpmsg_error(milu_lang('phpversion_too_hight_err').$status);
	}else{	
...
function pick_download(){
	...
	if($p == 0) {
		dir_clear($tmpdir);
		cache_del('new_version_md5total');
		...
		if(!$_GET['file_md5']){
			...
			$new_version_md5total = load_cache('new_version_md5total');
			...
			if($msg_arr['MD5']) {
				...
				cpmsg_error(milu_lang('no_normal_get').'error203');
			}
		...
		cache_data('new_version_md5total', $new_version_md5total);
		cpmsg(milu_lang('pick_upgrade_downloading_file', array('file' => $file, 'percent' => $percent)), PICK_GO.'pick_info&ac=pick_download&i='.($i+1).'&md5_total='.$version_md5total.'&key='.$key.'&p='.$p.'&version='.$version.'&version_dateline='.$version_dateline.'&count='.$count.'&file_md5='.$file_md5, 'loading', '', false);
	}else if($p == 2){
