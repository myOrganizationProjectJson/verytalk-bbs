...
function fastpick_set(){
	...
	if(!submitcheck('submit')) {
		...
		$info['fp_open_mod'][0] = in_array(1, $info['_fp_open_mod']) ? 1 : 0;//门户
		$info['fp_open_mod'][1] = in_array(2, $info['_fp_open_mod']) ? 1 : 0;//论坛
...
//DZ自身的远程图片本地化无法下载某些防盗链图片，这个函数用来完善
