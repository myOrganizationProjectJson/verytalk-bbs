...
function rules_edit(){
	...
	$copy =$_GET['pick_copy'];
	$setarr['url_var'] = pserialize($_POST['url_var']);
	...
	if($submit){
		...
		$url = PICK_GO.'system_rules&myac=rules_edit&rid='.$rid;
		...
		num_limit('strayer_rules', 30, 's_num_limit');
		...
		$data['url_var'] = dunserialize($data['url_var']);
		$data['title_filter_rules'] = dunserialize($data['title_filter_rules']);
		$data['content_filter_rules'] = dunserialize($data['content_filter_rules']);
		$data['reply_filter_rules'] = dunserialize($data['reply_filter_rules']);
		$data['forum_threadtypes'] = dunserialize($data['forum_threadtypes']);
...
function rules_import(){
	...
	if(submitcheck('addsubmit')) {
		...
		if(!array_key_exists('list_ID', $arr) && array_key_exists('detail_ID', $arr)) cpmsg_error(milu_lang('import_error3', array('url' => PICK_GO)));
		if(!$arr['rules_hash']){
		...
		if($check && !$update_flag){
			...
			cpmsg(milu_lang('cover_notice').'?<input type="hidden" value="'.dstripslashes($data).'" name="rules_code">', PICK_GO.'system_rules&myac=rules_import&addsubmit=1&update_flag=1', 'form');
		}
		if(!array_key_exists('is_setting_article_page', $arr)){
			$arr['is_setting_article_page'] = !empty($arr['content_page_rules']) ? 1 : 2;//是否设置分页
		}
		
			
		if(empty($arr['is_setting_article_page'])){//分页
			$arr['is_setting_article_page'] = !empty($arr['content_page_rules']) ? 1 : 2;
		} 
		if(empty($arr['is_get_reply'])){//回复
			$arr['is_get_reply'] = !empty($arr['reply_rules']) || $arr['reply_is_extend'] == 1 ? 1 : 2;
		} 
...
function create_variable(){
	...
	$count = count($arr);
	$data = get_rules_info($rid);
	$url_var = (array)unserialize($data['url_var']);
	for($i=1;$i<$count;$i++){
		...
		if($url_var[$i]['var_ext_step'][$i] == 1) $check_box = 'checked="checked" ';
...
<td>
  ...
  '.milu_lang('step_value').'<input  style="width:40px;margin-left:5px;" name="url_var['.$i.'][var_ext_step]['.$i.']" type="text" id="var_ext_step['.$i.']" value="'.$url_var[$i]['var_ext_step'][$i].'" /></label>
