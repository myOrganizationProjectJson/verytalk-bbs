...
class HtmlExtractor {
	...
	function get_normal_text(){
		...
		$max_info = $text_info = '';
		foreach((array)$this->text_arr as $k => $v){
	...
	function get_normal2(){
		...
		//$this->temp_dom_arr = '';
		foreach((array)$temp_dom_arr as $k => $v){
	...
	function get_img_text(){
		...
		foreach((array)$this->temp_dom_arr as $k => $v){
	...
	function remove_dom(){
		...
		foreach($this->dom_arr as $k => $v){
			...
			if($del_a_html && strexists($this->title, $del_a_html)){
	...
	function get_ori_title(){
		...
		$this->title = $title_arr[0] ? $title_arr[0] : $this->ori_title;
	...
	function format_result($text_info, $title_info){
		...
		$evo_result = (array)$this->evo_set($info);
	...
	function evo_set($info){
		...
		$link_count = intval($link_count);
		if(empty($_G['cache']['plugin']['milu_pick']['min_own_link'])) $_G['cache']['plugin']['milu_pick']['min_own_link'] = 10;
		if((int)$link_count > (int)$_G['cache']['plugin']['milu_pick']['min_own_link']) return FALSE;//有N个指向自己的链接，就不行
		$milu_set = pick_common_get();
