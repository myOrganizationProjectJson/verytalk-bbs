(function () {
    ...
    **/
    function GetJqObj(id) {
        return "string" == typeof (id) ? jQuery("#" + id) : jQuery(id);
    };
    ...
    **/
    function ShowTable(trJqObjArr) {
        for (var n = 0, len = trJqObjArr.length; n < len; n++) {
            ...
            jQuery.isFunction(_initConfig.OnShow) && (_initConfig.OnShow(n, trJqObjArr[n], _tbodyObj));
        }
    }
    ...
    **/
    function SetOrderItem(obj, index, options) {
        ...
        jQuery.extend(orderSettings, options);
        orderSettings.DataType = orderSettings.DataType.toLowerCase();
        obj = GetJqObj(obj);
        //绑定触发排序的事件
        obj.bind("click", function () {
            Sort(index, orderSettings);
            jQuery.isFunction(orderSettings.OnClick) && orderSettings.OnClick();
        });
    ...
    var _api = {
        Init: function (obj, tBodyIndex, options) {
            ...
            jQuery.extend(_initConfig, options);
